...
static int unify_exe_segments (int n_modules)
{
  ...
  for (int i = 0; i < n_modules; i++) {
	...
	continue;
      ...
      /* Weed out duplicates */
      for (int k = DEFAULT_SEGMENT + 1; k < index; k++) {
...
static int unify_bin_segments (int n_modules)
{
  ...
  *dest_seg = *src_seg;
  ...
  int index = DEFAULT_SEGMENT + 1;    
  int offset = src_seg->image_size - src_seg->escapes;
  ...
  for (int i = 1; i < n_modules; i++) {
    ...
    for (int k = 0; k < modules[i].lt.size; k++) {
      struct Label *lab = &modules[i].lt.labels[k];
      assert (lab->segment == DEFAULT_SEGMENT);
      if (lab->in_use && lab->defined)
	lab->address += offset;
    }
...
static int unify_symbols (int n_modules)
{
  ...
  for (int i = 0; i < n_modules; i++) {
    for (int j = 0; j < modules[i].lt.size; j++) {
      struct Label *lab = &modules[i].lt.labels[j];
      if (!lab->defined || !lab->in_use)
	continue;

      if (modules[i].lt.labels[j].export) {
	/* Weed out duplicates */
	for (int k = 0; k < index; k++) {
	  if (!strcmp(lab->name, all_sym.labels[k].name)) {
	    component_error(modules[i].name, "duplicate symbol", lab->name);
	    return EXIT_FAILURE;
	  }
	}
      }
	...
	lab->segment = modules[i].st.segments[seg].new_index;
      }
      
      /* Insert the symbol into the compound table */
      lab->new_index = index;
      all_sym.labels[index] = *lab;
  ...
  for (int i = 0; i < n_modules; i++)
	...
	for (int k = 0; k < index; k++) {
	  if (all_sym.labels[k].export
	      && !strcmp(all_sym.labels[k].name, lab->name)) {

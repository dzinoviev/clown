...
show_memory_action: '[' T_NUMBER ']' optformat T_EOL {
    show_range ($2, 1, $4); 
    return 0; 
...
set_memory_action: '[' T_NUMBER ']' '=' T_NUMBER T_EOL {
    set_mem ($2, $5); 
    return 0; 
};

load_memory_action: T_LOAD T_STRING optnumber T_EOL {
  fprintf (stderr, "%d words loaded\n", /* why does (int / uint) -> uint ???*/
	     load_memory ($2, $3) / (int)sizeof (Dword));
    free ($2);
    return 0; 
};
